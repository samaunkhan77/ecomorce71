...
<script>
    ...
    document.getElementById('load-more').addEventListener('click', function() {
        page++;
        axios.get(`/load-more-products?page=${page}`)
            .then(response => {
                if (response.data.html) {
                    document.getElementById('product-grid').insertAdjacentHTML('beforeend', response.data.html);
                } else {
                    document.getElementById('load-more').disabled = true;
                    document.getElementById('load-more').innerText = "No more products";
                }
            })
            .catch(error => {
                console.error(error);
